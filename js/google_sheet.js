...
function setupSectionStickyNav() {
  const nav = document.getElementById("sectionStickyNav");
  const tripNav = document.getElementById("tripStickyWrap");
  if (!nav || !tripNav) return;
  ...
  links.forEach(link => {
    link.addEventListener("click", event => {
      ...
      const offset = link.getAttribute("href") === "#tripStickyWrap" ? 0 : nav.offsetHeight;
      window.scrollTo({
  ...
  const updateNav = () => {
    ...
    nav.classList.toggle("is-hidden", tripNav.getBoundingClientRect().top <= 1);
