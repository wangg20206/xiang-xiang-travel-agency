...
function renderTours(data) {
  ...
  container.innerHTML = "";
  document.getElementById("tour-pagination")?.remove();
  ...
  setupTourPagination(container);
}

function setupTourPagination(container) {
  const cards = [...container.querySelectorAll(".tour-card")];
  if (!cards.length) return;

  const pagination = document.createElement("div");
  pagination.id = "tour-pagination";
  pagination.className = "tour-pagination";
  pagination.setAttribute("aria-label", "精選行程頁面");

  cards.forEach((card, index) => {
    const dot = document.createElement("button");
    dot.type = "button";
    dot.className = "tour-pagination-dot";
    dot.setAttribute("aria-label", `前往第 ${index + 1} 個精選行程`);
  ...
  container.addEventListener("scroll", () => {
    ...
    frameId = requestAnimationFrame(() => {
      ...
      pagination.querySelectorAll(".tour-pagination-dot").forEach((dot, index) => {
